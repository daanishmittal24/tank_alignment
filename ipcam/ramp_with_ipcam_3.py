...
def find_edge(contour, is_left_camera):
    ...
    if is_left_camera:
        leftmost_points = sorted(approx_contour, key=lambda x: x[0][0])[:2]
        edge_start, edge_end = sorted(leftmost_points, key=lambda x: x[0][1])
    else:
        rightmost_points = sorted(approx_contour, key=lambda x: x[0][0])[-2:]
        edge_start, edge_end = sorted(rightmost_points, key=lambda x: x[0][1])

    return edge_start[0], edge_end[0]
...
def process_frame(frame, predictor, mask_percentage, edge_history, is_left_camera):
    ...
    if ramp_instances.has("pred_masks") and len(ramp_instances.pred_masks) > 0:
        masks = ramp_instances.pred_masks.numpy()
        if len(masks) > 0:
            if is_left_camera:
                # Select the leftmost mask
                leftmost_mask = min(masks, key=lambda mask: np.min(np.nonzero(mask)[1]))
                mask = leftmost_mask
            else:
                # Select the rightmost mask
                rightmost_mask = max(masks, key=lambda mask: np.max(np.nonzero(mask)[1]))
                mask = rightmost_mask
            ...
            if contours:
                ...
                edge_start, edge_end = find_edge(largest_contour, is_left_camera)
                edge_history.append((edge_start, edge_end))
                if len(edge_history) == FRAME_HISTORY:
                    edge_start, edge_end = average_edge_position(edge_history)
                cv2.line(draw_frame, edge_start, edge_end, EDGE_COLOR, LINE_THICKNESS)
                ...
                vector_edge = np.array([edge_end[0] - edge_start[0], edge_end[1] - edge_start[1]])
                vector_green_line = np.array([0, -1])
                angle_degrees = calculate_angle(vector_edge, vector_green_line)  # Store the calculated angle
    ...
    if angle_degrees is not None:
        ...
        cv2.putText(draw_frame, "No ramp detected", (10, draw_frame.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, RED_COLOR,
                    1, cv2.LINE_AA)
...
while True:
    ...
    if current_time - last_process_time >= 1 / TARGET_FPS:
        ...
        if frame_copy1 is not None and frame_copy2 is not None:
            ...
            cv2.putText(processed_frame1, 'Left', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, GREEN_COLOR, 1, cv2.LINE_AA)
            cv2.putText(processed_frame2, 'Right', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, GREEN_COLOR, 1, cv2.LINE_AA)
